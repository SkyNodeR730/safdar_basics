...
class Analytics:
	...
	def apply_sales_person_filter(self, query, parent_doctype):
		if not self.filters.get("sales_person"):
			return query

		sales_team = DocType("Sales Team")
		return query.join(sales_team).on(sales_team.parent == parent_doctype.name).where(
			sales_team.sales_person == self.filters.sales_person
		)

	def get_sales_person_filters(self):
		if not self.filters.get("sales_person"):
			return []
	...
	def include_entities_with_zero_business(self):
		if self.filters.tree_type not in ("Customer", "Supplier"):
			return
	...
	def get_rows(self):
		...
		if not self.filters.get("sales_person"):
			self.include_entities_with_zero_business()
	...
	def get_rows_by_group(self):
		...
		if self.filters.get("sales_person"):
			out = [row for row in out if row["total"]]
