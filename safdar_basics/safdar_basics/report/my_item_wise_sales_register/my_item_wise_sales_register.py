...
def _execute(filters=None, additional_table_columns=None, additional_conditions=None):
	...
	if item_list:
		itemised_tax, tax_columns = get_tax_accounts(
			item_list, columns, company_currency, doctype="Sales Order"
		)
	...
	for d in item_list:
		...
		row = {
			...
			**get_values_for_columns(additional_table_columns, d),
			"sales_person": ", ".join(sales_persons.get(d.parent, [])),
			"territory": d.territory,
			"project": d.project,
			"company": d.company,
			"delivery_note": ", ".join(so_dn_map.get(d.name, [])),
...
def get_columns(additional_table_columns, filters):
	...
	columns.extend(
		[
			...
			{"label": _("Order Date"), "fieldname": "transaction_date", "fieldtype": "Date", "width": 120},
		]
	)
...
def apply_conditions(query, si, sii, filters, additional_conditions=None):
	...
	if filters.get("from_date"):
		query = query.where(si.transaction_date >= filters.get("from_date"))

	if filters.get("to_date"):
		query = query.where(si.transaction_date <= filters.get("to_date"))

	if filters.get("sales_person"):
		sales_orders = frappe.db.get_all(
			"Sales Team",
			{"sales_person": filters.get("sales_person"), "parenttype": "Sales Order"},
			pluck="parent",
		)
		query = query.where(si.name.isin(sales_orders or [""]))
...
def get_sales_persons(parent_names):
	...
	sales_team_rows = frappe.db.get_all(
		"Sales Team",
		filters={"parenttype": "Sales Order", "parent": ("in", list(parent_names))},
		fields=["parent", "sales_person"],
	)

	for d in sales_team_rows:
...
def get_delivery_notes_against_sales_order(item_list):
	...
	so_item_rows = list(set(d.name for d in item_list))
