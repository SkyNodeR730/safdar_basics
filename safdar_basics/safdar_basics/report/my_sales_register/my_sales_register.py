...
def get_conditions(filters, query, doctype):
	...
	if filters.get("custom_bill_location"):
		query = query.where(parent_doc.custom_bill_location == filters.custom_bill_location)

	if filters.get("custom_driver"):
		query = query.where(parent_doc.custom_driver == filters.custom_driver)

	if filters.get("custom_sales_person"):
		query = query.where(parent_doc.custom_sales_person == filters.custom_sales_person)

	if filters.get("status"):
		query = query.where(parent_doc.status == filters.status)
